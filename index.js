...
const collectUserEntry = async () => {
    ...
    while (!projectPath) {
        ...
        if (folderExists && !isPathAvailable(root, appName)) {
            console.error("The path already exists, please pick another name for your app");
        } else {
    ...
    console.log("");
    while (!openAIKey) {
        openAIKey = await getUserInput("OpenAI API Key?", null, validateString);
    }
    ...
    while (!pangeaServiceDomain) {
        pangeaServiceDomain = await getUserInput(
            "Pangea Domain?",
            null,
            validateString
        );
...
const run = async () => {
    ...
    console.log("We got what we need from you, now we will generate the Secure ChatGPT app! This might take short while so please be patient....");;
    console.log("");
    ...
    replaceInFile(envFile, "$NEXT_PUBLIC_PANGEA_DOMAIN$", userEntry.pangeaServiceDomain);
    replaceInFile(envFile, "$NEXT_PUBLIC_AUTHN_CLIENT_TOKEN$", userEntry.pangeaAuthClientToken);
    replaceInFile(envFile, "$NEXT_PUBLIC_AUTHN_HOSTED_LOGIN_URL$", userEntry.pangeaAuthClientHostedLoginURL);
    replaceInFile(envFile, "$PANGEA_SERVICE_TOKEN$", userEntry.pangeaServiceToken);
    replaceInFile(envFile, "$OPENAI_API_KEY$", userEntry.openAIKey);

    // Run `npm install` in the project directory to install
    spawnSync.spawnSync("npm", ["install"], { stdio: "inherit", cwd: userEntry.root });
